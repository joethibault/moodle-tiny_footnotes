define("tiny_footnotes/commands", [
    "exports",
    "core/str",
    "tiny_footnotes/common",
    "tiny_footnotes/ui"
], function(_exports, _str, _common, _ui) {
    "use strict";
    ...
    _exports.getSetup = function() {
        return Promise.all([
            _str.get_string("button_footnote", _common.component),
            _str.get_string("menuitem_footnote", _common.component)
        ]).then(function(labels) {
            var buttonLabel = labels[0];
            var menuItemLabel = labels[1];

            return function(editor) {
                editor.ui.registry.addIcon(_common.icon, iconSvg);

                editor.ui.registry.addButton(_common.buttonName, {
                    icon: _common.icon,
                    tooltip: buttonLabel,
                    onAction: function() { _ui.insertFootnote(editor); }
                });

                editor.ui.registry.addMenuItem(_common.menuItemName, {
                    icon: _common.icon,
                    text: menuItemLabel,
                    onAction: function() { _ui.insertFootnote(editor); }
                });

                editor.on("Change SetContent Undo Redo", function() {
                    _ui.renumber(editor);
                });
                ...
                editor.on("click", function(e) {
                    var ref = e.target.closest && e.target.closest("sup." + _common.cls.ref);
                    if (ref) {
                        _ui.focusNoteForRef(editor, ref);
                        e.preventDefault();
                    }
                });
            };
